...
public class Paginator implements Serializable {
	...
	public int getEndRow() {
		return pageIndex > 0 ? Math.min(pageSize * pageIndex, getTotalCount())
				: 0;
	}
	...
	public Integer[] slider(int slidersCount) {
		return generateLinkPageNumbers(getPageIndex(), (int) getTotalPages(),
				slidersCount);
	}
	...
	private static int computePageNumber(int page, int pageSize, int totalItems) {
		...
		if (Integer.MAX_VALUE == page
				|| page > computeLastPageNumber(totalItems, pageSize)) {
